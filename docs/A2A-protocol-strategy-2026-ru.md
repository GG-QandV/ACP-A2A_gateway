...
> **Язык:** русский · [English version](./A2A-protocol-strategy-2026.md)
...
- **Шлюз (`ACP-A2A_gateway`):** уже принимает SDK+Spec на входе (`transport_http.rs`). Добавить: детект по Agent Card/`protocolVersion`; вывод SDK-формата для SDK-запросов (частично в ТЗ `SPEC-add-adapterd-wire-format.md`).
