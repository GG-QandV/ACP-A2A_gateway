...
> **Язык:** русский · [English version](./decisions.md)
...
правок P1-1/P1-2/P2-8 и местами описывает код, которого уже нет (см. P-16).

---

## Протокол и типы

### P-1. Крейт называется `gateway-core`, не `core`
...
### P-2. `protocolVersion` — `u32`, приём либеральный, выход строгий
...
### P-3. `default` для версии — именованная функция, не `Default::default()`
...
### P-4. Клиентские возможности в `initialize` заявлены пустыми
...
и тогда же станет актуальна P-5.

### P-5. Reader отличает запросы агента от ответов по наличию `method`
...
слал (см. P-4), так что причиной наблюдавшегося зависания это, скорее
всего, не было — реальной причиной оказалась P-6. Фикс оставлен как
...
### P-6. `initialize` — часть подъёма процесса, а не отдельный вызов
...
### P-7. Респавн помечает потерянные разговоры, а не прячет их
...
поколение чужое и клиент получит ложный `ContextLost`. См. P-16.

### P-8. Респавн с backoff 5с, потолок 256 разговоров на агента
...
### P-9. Сессия на `contextId`, а не на токен
...
из-за того, что там написано (см. P-16).

### P-10. Владелец — хеш токена, не сам токен
...
### P-11. Владелец задачи — в конверте хранилища, не в `Task.metadata`
...
**Облако:** формат конверта переживёт переезд в БД; см. P-10 про
содержимое поля.

### P-12. `session/prompt` отклоняет неизвестный `sessionId`
...
### P-13. Уборка задач по mtime файла, а не по времени внутри задачи
...
### P-14. Коды отказа агента разделены по смыслу
...
### P-15. `AgentCard.url` — адрес конкретного агента, не шлюза
...
### P-16. Черновик облачной архитектуры частично устарел

`05-cloud-architecture-draft.md` §2.4 описывает
`session: Mutex<Option<SessionId>>` как текущее состояние. Этого поля
больше нет с P-9: состояние теперь `HashMap<ContextId, SessionEntry>`
плюс реестр сессий в `A2aAsAcp` (P-12) плюс кэш адаптеров плюс
`TurnLease`.
...
| `generation` процесса | `SupervisedStdioAgent` | Ложный `ContextLost` от чужого поколения (P-7) |
| `sessions` ACP-стороны | `A2aAsAcp` | `session/prompt` отвергнет валидную сессию (P-12) |
...
### P-17. TLS и rate limiting отданы reverse-proxy
...
### P-18. Стриминг не реализован — Фаза 2
...
### P-19. Не-ASCII `taskId` схлопываются в одно имя файла — оставлено как есть
...
вопросов и не требовал рукопожатия — поэтому три дефекта (P-6, порядок
в P-7, P-5) дожили до живого стенда. Каждый раз, когда фейк учили
