...
> **Язык:** русский · [English version](./streaming-roadmap-checklist.md)
Полный workflow от текущего состояния (P-18 в `docs/decisions-ru.md`: `Reply::Streaming` объявлен, но
...
- [ ] `cargo test --workspace` зелёный на текущем HEAD (69+ тестов, `docs/06-gateway-guide-ru.md` §2).
- [ ] `cargo clippy --workspace --all-targets -- -D warnings` без предупреждений.
- [ ] Прочитан и подтверждён P-18 в `docs/decisions-ru.md` — команда согласна, что `unreachable!()` в
...
- [ ] Заведён отдельный TECH_DEBT-пункт "стриминг в разработке" со ссылкой на этот роадмап, чтобы
      текущая запись TECH_DEBT-ru.md:17-21 не потерялась в процессе.
...
- [ ] **Критерий готовности 1.3**: live-прогон (как в `docs/06-gateway-guide-ru.md` §7 п.10 для
...
совместимость обязательна). Negative control по конвенции проекта (`docs/decisions-ru.md`, раздел
...
| T11 | Live | ручной прогон по образцу §7 п.10 `docs/06-gateway-guide-ru.md` | Реальный ACP-агент (claurst/hermes) стримит через направление 4 | Gate 2 |
| T12 | Clippy/build | `cargo clippy --workspace --all-targets -- -D warnings` | Ни один этап не оставляет warnings | Все этапы |

**Правило для всех тестов T1–T11** (конвенция проекта, `docs/decisions-ru.md`, раздел "Как это писалось"):
